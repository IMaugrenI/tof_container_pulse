...
SENSITIVE_PERMISSION_TARGETS = (("/etc/sudoers", 0o022), ("/etc/ssh/sshd_config", 0o022))
...
def _check_sessions(config):
    code, stdout, _stderr = _run_command(["who"], float(config.get("security_command_timeout_seconds", 1.5)))
    if code == 127:
    ...
    if count >= critical_at:
        return _entry("Active login sessions", "Aktive Login-Sitzungen", "Login", "Login", count, "critical", "critical", "Many terminal login sessions are active. Review whether this is expected. No session was changed.", "Viele Terminal-Login-Sitzungen sind aktiv. Prüfe, ob das erwartet ist. Es wurde keine Sitzung verändert.")
    if count >= warn_at:
        return _entry("Active login sessions", "Aktive Login-Sitzungen", "Login", "Login", count, "review", "warn", "Several terminal login sessions are active. This can be normal, but review if unexpected.", "Mehrere Terminal-Login-Sitzungen sind aktiv. Das kann normal sein, sollte aber geprüft werden, falls unerwartet.")
    if count > 0:
        return _entry("Active login sessions", "Aktive Login-Sitzungen", "Login", "Login", count, "active", "ok", "A small number of terminal login sessions is active. This is normal when you are logged in locally or using a terminal.", "Eine kleine Anzahl Terminal-Login-Sitzungen ist aktiv. Das ist normal, wenn du lokal angemeldet bist oder ein Terminal nutzt.")
...
def _read_tail_lines(path, max_lines):
    try:
        return Path(path).read_text(encoding="utf-8", errors="ignore").splitlines()[-max_lines:]
    except Exception:
        return None
...
def _check_auth_log(config):
    ...
    for candidate in AUTH_LOG_CANDIDATES:
        if Path(candidate).exists():
            ...
            lines = _read_tail_lines(candidate, int(config.get("security_auth_log_max_lines", 600)))
            break
    ...
    if failed >= int(config.get("security_failed_login_critical_count", 20)):
    ...
    if failed >= int(config.get("security_failed_login_warn_count", 5)):
...
def _check_fail2ban(config):
    code, stdout, stderr = _run_command(["fail2ban-client", "status"], float(config.get("security_command_timeout_seconds", 1.5)))
    if code == 127:
...
def _check_docker_socket(config):
    ...
    try:
        mode_value = path.stat().st_mode
        mode = oct(mode_value & 0o777)
    except Exception:
...
def _check_firewall_visibility(config):
    ...
    if code != 127:
        ...
        return _entry("Firewall visibility", "Firewall-Sichtbarkeit", "Network", "Netzwerk", "ufw available", "optional", "ok", "UFW exists, but its exact status could not be classified. This is an orientation hint; no rule was changed.", "UFW ist vorhanden, aber der genaue Status konnte nicht eingeordnet werden. Das ist ein Orientierungshinweis; keine Regel wurde verändert.")
...
def _check_docker_group(config):
    ...
    if count >= int(config.get("security_docker_group_warn_count", 4)):
...
def _check_sensitive_file_permissions(config):
    checked = suspicious = unreadable = 0
...
def _check_docker_container_audit(config):
    timeout = float(config.get("security_command_timeout_seconds", 1.5))
    code, stdout, stderr = _run_command(["docker", "ps", "-q"], timeout)
    if code == 127:
        return _entry("Docker container audit", "Docker-Container-Prüfung", "Containers", "Container", "docker not found", "optional", "ok", "Docker CLI was not detected. This is normal on hosts without Docker CLI access.", "Docker CLI wurde nicht erkannt. Das ist auf Hosts ohne Docker-CLI-Zugriff normal.")
    if code != 0:
        return _entry("Docker container audit", "Docker-Container-Prüfung", "Containers", "Container", "unavailable", "unknown", "unknown", f"Docker container metadata could not be read: {stderr or 'no output'}", f"Docker-Container-Metadaten konnten nicht gelesen werden: {stderr or 'keine Ausgabe'}")
    ids = [line.strip() for line in stdout.splitlines() if line.strip()]
    if not ids:
        return _entry("Docker container audit", "Docker-Container-Prüfung", "Containers", "Container", "0 running", "ok", "ok", "No running containers were found for this read-only audit.", "Für diese nur lesende Prüfung wurden keine laufenden Container gefunden.")
    max_count = int(config.get("security_docker_inspect_max_containers", 30))
    ids = ids[:max_count]
    code, inspect_out, stderr = _run_command(["docker", "inspect", *ids], timeout)
    if code != 0:
        return _entry("Docker container audit", "Docker-Container-Prüfung", "Containers", "Container", f"{len(ids)} listed", "unknown", "unknown", f"Docker inspect could not be read: {stderr or 'no output'}", f"Docker inspect konnte nicht gelesen werden: {stderr or 'keine Ausgabe'}")
    try:
        containers = json.loads(inspect_out)
    except Exception:
        return _entry("Docker container audit", "Docker-Container-Prüfung", "Containers", "Container", f"{len(ids)} listed", "unknown", "unknown", "Docker inspect output could not be parsed.", "Docker-inspect-Ausgabe konnte nicht gelesen werden.")
    risky = 0
    hints = []
    for item in containers:
        ...
        user = str(config_data.get("User") or "").strip()
        if host.get("Privileged"):
            risky += 1; hints.append("privileged")
        if network_mode == "host":
            risky += 1; hints.append("host-network")
        if str(host.get("PidMode") or "") == "host" or str(host.get("IpcMode") or "") == "host":
            risky += 1; hints.append("host-namespace")
        if any("/var/run/docker.sock" in str(bind) for bind in binds):
            risky += 1; hints.append("docker-socket-mount")
        if user in {"", "0", "root"}:
            hints.append("root-user-common")
    if risky:
        unique = ", ".join(sorted(set(hints))[:4])
        return _entry("Docker container audit", "Docker-Container-Prüfung", "Containers", "Container", f"{risky} review", "review", "warn", f"Some running containers use powerful settings: {unique}. Review intentionally. No container was changed.", f"Einige laufende Container nutzen starke Einstellungen: {unique}. Prüfe das bewusst. Es wurde kein Container geändert.")
    return _entry("Docker container audit", "Docker-Container-Prüfung", "Containers", "Container", f"{len(containers)} checked", "calm", "ok", "No privileged, host-network, host-namespace, or Docker-socket-mounted running containers were found in the inspected set.", "In der geprüften Menge wurden keine privilegierten Container, Host-Netzwerke, Host-Namespace-Nutzung oder Docker-Socket-Mounts gefunden.")


def _check_ssh_key_hygiene(config):
    ssh_dir = Path.home() / ".ssh"
    if not ssh_dir.exists():
        return _entry("SSH key hygiene", "SSH-Key-Hygiene", "Login", "Login", "no .ssh", "optional", "ok", "No .ssh directory was found for the current user.", "Für den aktuellen Nutzer wurde kein .ssh-Verzeichnis gefunden.")
    suspicious = 0
    checked = 0
    ...
    if auth_file.exists():
        ...
        try:
            ...
            pass
    if suspicious:
        return _entry("SSH key hygiene", "SSH-Key-Hygiene", "Login", "Login", f"{suspicious}/{checked}", "review", "warn", "Some current-user SSH files appear more open than expected. File names and contents are not shown.", "Einige SSH-Dateien des aktuellen Nutzers wirken offener als erwartet. Dateinamen und Inhalte werden nicht angezeigt.")
    return _entry("SSH key hygiene", "SSH-Key-Hygiene", "Login", "Login", f"{checked} checked", "calm", "ok", "Current-user SSH file permissions look calm. Key contents and file names are not shown.", "Die SSH-Dateirechte des aktuellen Nutzers wirken ruhig. Schlüssel-Inhalte und Dateinamen werden nicht angezeigt.")


def _check_lsm_visibility(config):
    code, stdout, _stderr = _run_command(["aa-status"], float(config.get("security_command_timeout_seconds", 1.5)))
    if code != 127:
        text = stdout.lower()
        if "apparmor is enabled" in text or "profiles are loaded" in text:
            return _entry("AppArmor/SELinux visibility", "AppArmor/SELinux-Sichtbarkeit", "Protection", "Schutz", "apparmor visible", "active", "ok", "AppArmor visibility is available and appears active.", "AppArmor ist sichtbar und wirkt aktiv.")
        return _entry("AppArmor/SELinux visibility", "AppArmor/SELinux-Sichtbarkeit", "Protection", "Schutz", "apparmor available", "optional", "ok", "AppArmor tooling is available, but the status was not classified.", "AppArmor-Werkzeug ist vorhanden, aber der Status wurde nicht eingeordnet.")
    selinux_path = Path("/sys/fs/selinux/enforce")
    if selinux_path.exists():
        try:
            value = selinux_path.read_text(encoding="utf-8").strip()
        except Exception:
            return _entry("AppArmor/SELinux visibility", "AppArmor/SELinux-Sichtbarkeit", "Protection", "Schutz", "selinux unreadable", "unknown", "unknown", "SELinux marker exists but could not be read.", "SELinux-Marker existiert, konnte aber nicht gelesen werden.")
        return _entry("AppArmor/SELinux visibility", "AppArmor/SELinux-Sichtbarkeit", "Protection", "Schutz", "selinux enforcing" if value == "1" else "selinux permissive", "active" if value == "1" else "optional", "ok", "SELinux visibility marker was found. No setting was changed.", "SELinux-Sichtbarkeitsmarker wurde gefunden. Es wurde nichts verändert.")
    return _entry("AppArmor/SELinux visibility", "AppArmor/SELinux-Sichtbarkeit", "Protection", "Schutz", "not detected", "optional", "ok", "No common AppArmor or SELinux visibility marker was detected. This is an orientation hint only.", "Kein üblicher AppArmor- oder SELinux-Sichtbarkeitsmarker wurde erkannt. Das ist nur ein Orientierungshinweis.")


def _check_systemd_failed_units(config):
    code, stdout, _stderr = _run_command(["systemctl", "--failed", "--no-legend", "--plain"], float(config.get("security_command_timeout_seconds", 1.5)))
    if code == 127:
        return _entry("Failed systemd units", "Fehlgeschlagene systemd-Units", "Maintenance", "Wartung", "systemctl not found", "optional", "ok", "systemctl was not found. This is normal outside systemd systems.", "systemctl wurde nicht gefunden. Das ist außerhalb von systemd-Systemen normal.")
    if code != 0:
        return _entry("Failed systemd units", "Fehlgeschlagene systemd-Units", "Maintenance", "Wartung", "unavailable", "unknown", "unknown", "Failed units could not be read in this environment.", "Fehlgeschlagene Units konnten in dieser Umgebung nicht gelesen werden.")
    count = len([line for line in stdout.splitlines() if line.strip()])
    if count >= int(config.get("security_systemd_failed_critical_count", 5)):
        return _entry("Failed systemd units", "Fehlgeschlagene systemd-Units", "Maintenance", "Wartung", count, "critical", "critical", "Several failed systemd units were reported. Review manually. Unit names are not shown.", "Mehrere fehlgeschlagene systemd-Units wurden gemeldet. Prüfe manuell. Unit-Namen werden nicht angezeigt.")
    if count >= int(config.get("security_systemd_failed_warn_count", 1)):
        return _entry("Failed systemd units", "Fehlgeschlagene systemd-Units", "Maintenance", "Wartung", count, "review", "warn", "One or more failed systemd units were reported. Review manually. Unit names are not shown.", "Eine oder mehrere fehlgeschlagene systemd-Units wurden gemeldet. Prüfe manuell. Unit-Namen werden nicht angezeigt.")
    return _entry("Failed systemd units", "Fehlgeschlagene systemd-Units", "Maintenance", "Wartung", "0", "calm", "ok", "No failed systemd units were reported.", "Es wurden keine fehlgeschlagenen systemd-Units gemeldet.")


def _collect_entries(config):
    entries = [_check_hostname()]
    checks = [
        ("security_check_sessions", _check_sessions),
        ("security_check_auth_log", _check_auth_log),
        ("security_check_fail2ban", _check_fail2ban),
        ("security_check_docker_socket", _check_docker_socket),
        ("security_check_reboot_required", _check_reboot_required),
        ("security_check_firewall_visibility", _check_firewall_visibility),
        ("security_check_ssh_config", _check_ssh_config),
        ("security_check_docker_group", _check_docker_group),
        ("security_check_sensitive_file_permissions", _check_sensitive_file_permissions),
        ("security_check_docker_container_audit", _check_docker_container_audit),
        ("security_check_ssh_key_hygiene", _check_ssh_key_hygiene),
        ("security_check_lsm_visibility", _check_lsm_visibility),
        ("security_check_systemd_failed_units", _check_systemd_failed_units),
    ]
    for key, func in checks:
        if bool(config.get(key, True)):
            entries.append(func(config))
    ...
    return f"""
      <section class="health-panel" aria-label="Security signal details">
        ...
        <section class="table-shell" aria-label="Security signals table">
          <table>
            <thead><tr><th>{l10n_text('Signal', 'Signal')}</th><th>{l10n_text('Area', 'Bereich')}</th><th>{l10n_text('Value', 'Wert')}</th><th>{l10n_text('State', 'Zustand')}</th><th>{l10n_text('Severity', 'Status')}</th><th>{l10n_text('Plain-English help', 'Einfache Erklärung')}</th></tr></thead>
            <tbody>{rows}</tbody>
          </table>
        </section>
      </section>
